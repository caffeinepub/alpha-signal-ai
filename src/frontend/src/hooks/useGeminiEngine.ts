...
function analyzeCandles(
  ...
  if (candles.length < 5) {
    return {
      analysisText: "Neutral — Gemini 1.5 Pro awaiting confluence signal",
      signal: "NEUTRAL",
      geminiConfidence: 50,
      ema50: 0,
      ema200: 0,
  ...
  if (rsi14 < 30 && nearLow) {
    signal = "STRONG BUY";
    analysisText =
      "Strong Buy — Gemini 1.5 Pro identifies oversold liquidity sweep";
  } else if (rsi14 > 70 && nearHigh) {
    signal = "STRONG SELL";
    analysisText =
      "Strong Sell — Gemini 1.5 Pro detects overbought distribution zone";
  } else if (currentPrice > ema50 && currentPrice > ema200) {
    signal = "BUY";
    analysisText = "Buy — Gemini 1.5 Pro confirms bullish trend continuation";
  } else if (currentPrice < ema50 && currentPrice < ema200) {
    signal = "SELL";
    analysisText = "Sell — Gemini 1.5 Pro confirms bearish trend continuation";
  } else {
    signal = "NEUTRAL";
    analysisText = "Neutral — Gemini 1.5 Pro awaiting confluence signal";
  }
