...
export default function Signals() {
  ...
  const signalOverride = localStorage.getItem("adminSignalOverride") as
    | "AUTO"
    | "BUY"
    | "SELL"
    | "WAIT"
    | null;
  const marketStatus = localStorage.getItem("marketStatus") || "Trending";
  const geminiEnabled = localStorage.getItem("geminiEnabled") !== "false";
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {signalOverride && signalOverride !== "AUTO" && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg px-4 py-2 text-amber-400 text-xs font-bold">
          ⚠️ Admin Override Active: All signals showing {signalOverride}
        </div>
      )}
      ...
      {/* Gemini disabled notice */}
      {!geminiEnabled && (
        <div className="bg-zinc-800/50 border border-zinc-700/40 rounded-lg px-4 py-2 text-zinc-400 text-xs">
          🤖 AI prediction section is currently disabled by admin.
        </div>
      )}
